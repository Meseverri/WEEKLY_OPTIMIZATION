   ...
   if(barsTotal<bars)
     {
         ...
         if(firstCrossOver && fastEMA[0]>slowEMA[0] && fastEMA[1]<=slowEMA[1])
           {
            ...
            SLPrice         = iHigh(_Symbol,Timeframe,newShift);
         ...
         if(firstCrossUnder && fastEMA[0]<slowEMA[0] && fastEMA[1]>=slowEMA[1])
           {
            ...
            SLPrice         = iLow(_Symbol,Timeframe,newShift);
      ...
      double close2=iHigh(_Symbol,Timeframe,1);
      double high1=iHigh(_Symbol,Timeframe,2); // Penultimo
      double high0=iHigh(_Symbol,Timeframe,3);// Antepenultimo


      double low1=iLow(_Symbol,Timeframe,2); // Penultimo
      double low0=iLow(_Symbol,Timeframe,3);// Antepenultimo
      ...
      if(firstPivot && AppPriceVWAP==PRICE_HIGH && high1>VWAP[1])//  && high0<VWAP[0] && close2>VWAP[2])
        {
         ...
         AppPriceVWAP=PRICE_LOW;
         changeVWAP(pivotTime, AppPriceVWAP,"parameters.csv", handleVWAP);
...
//Condiciones necesaria para cambiar el vwap de sentido
