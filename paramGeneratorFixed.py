...
def ParamEstimator(ref,relationMatrix,allMedianResults,Estrategy=1,dropFistYear=False):
    ...
    for i in years:
        ...
        print(Ref)
        newRow.append(relationMatrix.loc[Ref[0]:Ref[1],Ref[2]].idxmin())
        if flag52: ref = 53
        
    refWeek = pd.DataFrame()
    if not(dropFistYear):
        refWeek=pd.concat([allMedianResults[newRow[0][0],newRow[0][1]],
        allMedianResults[newRow[1][0],newRow[1][1]],
        allMedianResults[newRow[2][0],newRow[2][1]],
        allMedianResults[newRow[3][0],newRow[3][1]],
        allMedianResults[newRow[4][0],newRow[4][1]]],axis=1).iloc[8:,:].T
    else:
        refWeek=pd.concat([allMedianResults[newRow[0][0],newRow[0][1]],
        allMedianResults[newRow[1][0],newRow[1][1]],
        allMedianResults[newRow[2][0],newRow[2][1]],
        allMedianResults[newRow[3][0],newRow[3][1]]],axis=1).iloc[8:,:].T
...
def ParamEstimator2(ref,relationMatrix,allMedianResults,Estrategy=2,dropFistYear=False):
    Ref=refTupleGenerator(2023,ref,Estrategy)
    print(Ref)
...
for i in range(1,53):
    _completeParams = dictionaryInputs | ParamEstimator(i,RelationMatrix,AllBestData,Estrategy=4,dropFistYear=True) 
    if i > 0 and i <10: i = f"0{i}"
    f = open(f"2023_BT_sets/W{i}.set","w", encoding='utf-16')
